...
const HomePage = () => {
    return (
        ...
        >
            <SEO
                title="Realty Tunax | Yeni Nesil Gayrimenkul Danışmanlığı"
                description="Realty Tunax ile Antalya'da satılık ve kiralık gayrimenkul portföylerini keşfedin. Uzman ekibimizle yatırım ve yaşam alanı çözümleri sunuyoruz."
                keywords={['Realty Tunax', 'Antalya emlak', 'gayrimenkul danışmanlığı', 'satılık daire']}
            />
            {/* Hero Section */}
            <section
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
                        {featureItems.map(item => (
                            <div key={item.title} className="flex flex-col items-center text-center p-6 sm:p-8 bg-white rounded-xl shadow-sm hover:shadow-lg transition-shadow duration-300">
                                <img className="bg-rose-100 p-1 sm:p-1 my-auto" src={item.photo} alt={item.title} loading="lazy" />
